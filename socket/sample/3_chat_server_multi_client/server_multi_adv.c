...
void broadcast_message(char *msg, int msg_len, int sender_fd) {
    pthread_mutex_lock(&lock);
    char wraperMsg[BUFFER_SIZE+1];
    memcpy(wraperMsg+1, msg, msg_len); // 1 is ip of client sender
    int total_len = msg_len + 1;
    for (int i = 0; i < client_count; i++) {
        wraperMsg[0] = '0' + sender_fd;
        if (clients[i] != sender_fd) { // Forward message for all client except client's message.
            send(clients[i], wraperMsg, total_len, 0);
        }
    }
...
void *handle_client(void *arg) {
    ...
    while ((valread = recv(client_fd, buffer, sizeof(buffer) - 1, 0)) > 0) {
        ...
        broadcast_message(buffer, valread, client_fd);
